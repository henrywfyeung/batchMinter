...
const tokenVaultPk = process.env.PRIVATE_KEY;
...
const tokenVaultSigner = new ethers.Wallet(tokenVaultPk, provider);

console.log(`My Balance = ${await tokenVaultSigner.getBalance()}`);
...
if (stage === "fund") {
    ...
    const initialise_wallets = async (mnemonic, wallet_num, from_wallet) => {
        let walletList = [];
        for ( let i = 0 ; i < wallet_num ; i ++ ) {
            ...
            if (balance.lt(ethers.utils.parseEther(fund))) {
                console.log("Transferring ether to address" + (i+1).toString());
    ...
    walletList = await initialise_wallets(mnemonic, walletNumber, tokenVaultSigner);
}
